...
and two extra private subnet in 1c as well **DONE
...
Go ahead and create it. please make sure it has the extension .pem **DONE

5– We will have a server in the private subnet with no ssh access. go ahead and create a bastion host, copy 

the key pair ( utc-key) to it so we can access the private server.
...
also make sure that apache is running and able to render content. **DONE, but needs to be checked
...
check with the config management team if a domain exists or if we need to create one from scratch. ( as we

 already have a hosted zone in aws and we just need a subdomain called learning.yourdomain)
 ...
 **PARTIALLY DONE ; need ip to create subdomain

11- We need to create a load balancer. ( ALB)

listener: port 80

targetgroup: utc-target-group **DONE
...
password: utcdev12345 **DONE, but needs to be checked

13- Create an IAM role to give access from ec2 to s3 bucket **DONE

14- create an efs volume and attach it to both app-servers. **DONE, but needs to be checked

15- create a cron job in the appserver to upload the httpd log files into s3 bucket. *DONE, check plz

16- Create an ami from any of the app-server so we don’t have to repeat the setup on the next ec2 launch. 

call it utcappserver **DONE, but needs to be checked
...
key_name: utc-key **DONE, but needs to be checked

18- create an auto scaling group for utc-target-group with dynamic acalling base on cpu utilisation.

we should scale out when cpu reach 80 and scale back in wgen cpu goes down. **DONE, check plz

19- create a topic in sns service called utc-auto-scaliing. also create a subscription for that topic with the 

team email and make it configure autoscaling notification using that topic. **DONE

20- Start conversation around automating all this using terraform.

we might need to create multiple modules for this infrastructure.
